...
The timing for this is not strict, around a couple of seconds, so just make sure to accurately and swiftly select your badge of choice and you shouldn't have to worry about any additional timeloss.

#### Badge Equip Prompts

Because swapping badges loses no time when loading into a level, that means **we almost never want to swap in any other way.**
...
So, for all Badge Challenge IIs (and both of the Parachute Cap Badge Challenges), you use the R button on the World Map to quickly swap to the specific badge, then enter the challenge. Your swap timing *does* matter here for saving the most amount of time, but even mediocre execution can still **save you over 1s, typically 1.5s.**